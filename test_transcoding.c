...
int prepare_video_encoder(StreamingContext *sc, AVCodecContext *decoder_ctx, AVRational input_framerate, StreamingParams sp) {
  ...
  av_opt_set(sc->video_avcc->priv_data, "preset", "8", 0); //earlier instead of 8 for H264 preset is fast
  ...
  sc->video_avcc->bit_rate = 1000 * 1000;
...
int main(int argc, char *argv[])
{
  ...
  sp.copy_audio = 1;
  sp.copy_video = 0;
  sp.video_codec = "libsvtav1";//libaom-av1"; //"libx265";
  sp.codec_priv_key = "svtav1-params";
  //sp.codec_priv_value = "keyint=300:min-keyint=150:scenecut=0";

  StreamingContext *decoder = (StreamingContext*) calloc(1, sizeof(StreamingContext));
  decoder->filename = "{Enter path to input file}"; 

  StreamingContext *encoder = (StreamingContext*) calloc(1, sizeof(StreamingContext));
  encoder->filename = "{Enter path to output file}";
